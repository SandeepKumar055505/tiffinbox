...
Triggered when admin approves a post-cutoff skip request.

```typescript
boss.work(DomainEvent.MEAL_SKIPPED, async (job) => {
  const { meal_cell_id, user_id, amount, skip_type } = job.data;
  // skip_type: 'pre_cutoff' (auto) or 'post_cutoff' (admin approved)
  // 1. INSERT ledger_entries (direction='credit', description='Skip applied — ₹X added to wallet')
  // 2. UPDATE meal_cells SET delivery_status='skipped', wallet_credited=true
...
boss.work('streak.daily-update', async () => {
  ...
  // 1. Check if all included meals were delivered (not failed/skipped)
  // 2. If yes: increment current_streak, update longest_streak if needed
...
boss.work(DomainEvent.PAYMENT_SUCCESS, async (job) => {
  ...
  // 3. If wallet_applied > 0: INSERT ledger_entries (debit, 'Wallet used at checkout')
